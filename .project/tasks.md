...
| ID | Tarea | Depende de | Estado | Criterio de aceptación |
|---|---|---|---|---|
| T-033 | Edge Function `verify-theft` (adapter `TheftCheckProvider` vs. API "Auto Seguro") | T-012 | En progreso | **Hallazgo importante**: no existe una API pública del "Auto Seguro" para terceros (ver architecture.md §8, corregido) — implementado con un `TheftCheckProvider` **mock** explícito (patentes que empiezan con "ROBO" simulan encargo) para no bloquear el resto del pipeline. Función desplegada a Supabase; no probada en vivo (falta el service_role key para invocarla con autenticación `secret`) |
| T-034 | Tabla `jobs` + Edge Function `process-jobs` (Cron 5 min, retry con backoff exponencial) | T-016 | En progreso | Desplegada, con el schedule de backoff de architecture.md §9. No probada en vivo; **cron no conectado todavía** (pg_cron+pg_net requieren el service_role key para autenticar la llamada HTTP) |
...
| T-036 | Transición automática a `activo` cuando robo=OK **y** identidad=verificada | T-035, T-028 | En progreso | Función `try_activate_listing()` verificada con smoke test real: no activa con ningún gate, no activa con solo robo OK, sí activa con ambos gates OK (identidad simulada manualmente ya que T-028/KYC sigue bloqueado por Truora). Falta la invocación real desde `process-jobs` en producción (ver T-033) |
| T-037 | Bloqueo inmediato si `hasTheftReport=true` | T-035 | En progreso | Lógica escrita en `process-jobs` (status→`rechazado` + `audit_logs`); no probada en vivo por la misma razón que T-033 |
| T-038 | Cron `reverify-active-listings` (cada 24h) | T-034, T-036 | En progreso | Función desplegada (evita duplicar jobs ya encolados); cron no conectado todavía (mismo bloqueo del service_role key) |
| T-039 | Desactivación automática + notificación si re-verificación falla | T-038 | En progreso | Desactivación + `audit_logs` implementados. **Notificación al vendedor no implementada** — depende de Resend (T-008, diferido) |
