...
- **Robo → proveedor sin confirmar (corregido, agosto 2026)**: la suposición original de que `autoseguro.gob.cl` es una API pública documentada era incorrecta. Es un **formulario web para consumidores** (patente/VIN/motor), sin documentación de desarrollador, sin términos de uso para consultas automatizadas, sin endpoints publicados. "API Autoseguro" es el nombre del sistema de interconexión *entre instituciones* (Carabineros, PDI, Registro Civil), no un servicio abierto a terceros. **Dato a favor**: el informe de Autofact (ya evaluado para el CAV, más abajo) según su propia descripción incluye reportes de robo — el gate de robo y el CAV podrían resolverse con la misma negociación comercial, en vez de dos integraciones separadas. Hasta confirmar eso, el adapter `TheftCheckProvider` (T-033) se implementa con un mock explícito, dejando toda la mecánica (cola de verificación, gate, re-verificación cada 24h) lista para conectar el proveedor real sin rediseñar nada.
...
| Robo | Sin proveedor confirmado (ver §8) — probable mismo contrato que Autofact/CAV | Desconocido, no US$0 como se asumió originalmente |
...
**Riesgo de presupuesto identificado**: a diferencia de Vercel/Supabase/Sentry/Resend (con costos conocidos y acotados), **KYC, CAV y ahora Robo son las tres partidas sin precio confirmado** (Robo se agregó en agosto 2026 al descubrir que no existe una API pública gratuita, corrigiendo el supuesto original de esta sección). KYC en particular es un costo por usuario registrado (no por transacción), lo que lo hace menos predecible que el resto. No lanzar a producción con tráfico real sin haber cotizado los tres — y validar primero si Robo y CAV se resuelven con un solo contrato (Autofact), lo que simplificaría esto a dos negociaciones en vez de tres.
